...
func testSetup() (chan *nats.Msg, chan *nats.Msg) {
	...
	nc.ChanSubscribe("elb.update.aws.done", doneChan)
	nc.ChanSubscribe("elb.update.aws.error", errChan)
	...
	Convey("Given I an event", t, func() {
		Convey("With valid fields", func() {
			...
			Convey("When completing the event", func() {
				...
				e.Process(valid)
				e.Complete()
				Convey("It should produce a elb.update.aws.done event", func() {
			...
			Convey("When erroring the event", func() {
				...
				Convey("It should produce a elb.update.aws.error event", func() {
					msg, timeout := waitMsg(errored)
					So(msg, ShouldNotBeNil)
		...
		Convey("With an invalid elb from port", func() {
			...
			Convey("When validating the event", func() {
				...
				Convey("It should error", func() {
					So(err, ShouldNotBeNil)
					So(err.Error(), ShouldEqual, "ELB from port invalid")
				})
			})
		})
		...
		Convey("With an invalid elb to port", func() {
			...
			Convey("When validating the event", func() {
				...
				Convey("It should error", func() {
					So(err, ShouldNotBeNil)
					So(err.Error(), ShouldEqual, "ELB to port invalid")
				})
			})
		})
		...
		Convey("With an invalid protocol", func() {
			...
			Convey("When validating the event", func() {
				...
				Convey("It should error", func() {
					So(err, ShouldNotBeNil)
					So(err.Error(), ShouldEqual, "ELB protocol invalid")
				})
			})
		})
